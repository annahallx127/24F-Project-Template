    ...
    if job_id:
        ...
        if not payload:
            ...
            with st.spinner("Updating job listing..."):
                try:
                    response = requests.put(f"http://web-api:4000/hm/hiring-manager/job-listings/{job_id}", json=payload)
    ...
    if job_id_to_delete:
        try:
            with st.spinner(f"Deleting job listing with ID {job_id_to_delete}..."):
                response = requests.delete(f"http://web-api:4000/hm/hiring-manager/job-listings/{job_id_to_delete}")
                if response.status_code == 200:
